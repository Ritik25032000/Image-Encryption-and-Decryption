...
class database:
    ...
    #
    def insertion_in_encrypt(fname,ing_key,year,month,day):
        con=mysql.connector.connect(host="localhost",user="root",password="Ritik@98765",database="imageED")
        cur = con.cursor()
        query="Insert into ecp values({},'{}',{},{},{})".format(ing_key,fname,year,month,day)
        cur.execute(query)
        con.commit()

    #
    def insertion_in_decrypt(fname,ing_key,year,month,day):
        con=mysql.connector.connect(host="localhost",user="root",password="Ritik@98765",database="imageED")
        cur = con.cursor()
        query="Insert into dcp values({},'{}',{},{},{})".format(ing_key,fname,year,month,day)
        cur.execute(query)
        con.commit()
    ...
    def ret_from_encrypt_key(fname):
        con=mysql.connector.connect(host="localhost",user="root",password="Ritik@98765",database="imageED")
        cur = con.cursor()
        query = "select ing_key from ecp where Fname='{}'".format(fname)
        cur.execute(query)
        data=cur.fetchall()
        x=cur.rowcount
        str1=""
    ...
    def ret_from_encrypt_file(key):
        con=mysql.connector.connect(host="localhost",user="root",password="Ritik@98765",database="imageED")
        cur = con.cursor()
        query = "select Fname from ecp where ing_key={}".format(key)
        cur.execute(query)
        data=cur.fetchall()
        x=cur.rowcount
        str1=""
        if x==0:
            return "There is no file present having ing_key ={}".format(key)
        else:
            for i in data:
                str1+=i[0]
                str1+="\n"
    ...
    def delete_in_encrypt(fname,key):
        con=mysql.connector.connect(host="localhost",user="root",password="Ritik@98765",database="imageED")
        cur = con.cursor()
        query = "Delete from ecp where fname='{}' and ing_key={}".format(fname,key)
        cur.execute(query)
        con.commit()
